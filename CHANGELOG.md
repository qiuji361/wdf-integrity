...
- 机型目录扩展到 131 款，本轮新增 16 款机型改包（OPPO/vivo 新机型）。
- cpuwz 升级到 v1.10-exp 离线公共候选，44 个 profile：9 个处理器+显卡一起改，34 个只改显卡，1 个锁定。
- 大白话功能使用说明全文重写，去掉技术术语，新增重启生效说明和重启前需先关闭的功能说明。
- 处理器/显卡伪装模块重点补充"处理器和显卡最好一起改"的说明和推荐配置。
- 高级防误判、游戏防标记、设备标识、广告标识清理、应用冻结隐藏、游戏缓存清理全功能真机验收通过。
